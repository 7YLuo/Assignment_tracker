...
function corsHeaders(request: Request) {
  ...
  return {
    'Access-Control-Allow-Origin': allowedOrigins.has(origin) ? origin : 'https://7yluo.github.io',
...
Deno.serve(async (request) => {
  ...
  if (!request.headers.get('authorization')) return json(request, { error: '请先登录。' }, 401);

  const apiKey = Deno.env.get('OPENAI_API_KEY');
  if (!apiKey) return json(request, { error: 'AI 服务尚未配置 OPENAI_API_KEY。' }, 503);

  let body: { text?: string; imageDataUrl?: string };
  try {
    body = await request.json();
  } catch {
    return json(request, { error: '无法读取提交内容。' }, 400);
  }

  const syllabusText = typeof body.text === 'string' ? body.text.trim().slice(0, 60000) : '';
  const imageDataUrl = typeof body.imageDataUrl === 'string' ? body.imageDataUrl : '';
  if (!syllabusText && !imageDataUrl) return json(request, { error: '请粘贴 syllabus 文字或添加一张截图。' }, 400);
  if (imageDataUrl && !/^data:image\/(png|jpeg|webp);base64,/i.test(imageDataUrl)) return json(request, { error: '只支持 PNG、JPG 或 WebP 图片。' }, 400);
  if (imageDataUrl.length > 8_000_000) return json(request, { error: '图片太大，请压缩到 6 MB 以内。' }, 413);
  ...
  if (!outputText) return json(request, { error: 'AI 没有返回可导入的课程信息。' }, 502);

  try {
    return json(request, JSON.parse(outputText));
  } catch {
    return json(request, { error: 'AI 返回格式无效，请重试。' }, 502);
  }
});
